...
export default function MyBuildings() {
  ...
  useEffect(() => {
    fetch(`http://localhost:8000/admin-buildings?admin_id=${userId}`)
      ...
      .catch((err) =>
        console.error("Failed to load assigned buildings", err)
      );
  }, [userId]);
  ...
  const handleAssign = () => {
    ...
    fetch("http://localhost:8000/admin-buildings", {
      ...
      body: JSON.stringify({ admin_id: userId, building_id: selectedBuildingId }),
    })
      .then((res) => res.json())
      .then(() => {
        setSelectedState("");
        ...
        return fetch(`http://localhost:8000/admin-buildings?admin_id=${userId}`);
      })
      .then((res) => res.json())
      .then(setAssignedBuildings)
