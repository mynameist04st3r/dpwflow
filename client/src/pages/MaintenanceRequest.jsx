...
const MaintenanceRequestPage = () => {
  ...
  useEffect(() => {
    const matchingBases = allLocations.filter(
      (loc) => loc.state === formData.state
    );
  ...
  useEffect(() => {
    const match = baseOptions.find(
      (loc) => loc.military_base === formData.military_base
    );
    if (match) {
      setFormData((prev) => ({
        ...prev,
  ...
  useEffect(() => {
    ...
    const storedLocationId = parseInt(
    ...
    <div className="maint-request-container">
      <header className="home-header">
        ...
        <p className="subtitle">
          Please fill out all of the fields and be as accurate as possible.
        </p>
        <p className="subtitle">
          To better track any requests, please create an account and then submit
          your request.
        </p>
        <p className="subtitle">
          Please do NOT submit a request and then create an account/login and
          submit another.
        </p>
      </header>
      <form onSubmit={handleSubmit} className="maint-request-form">
        ...
        >
          ...
          {[...new Set(allLocations.map((loc) => loc.state))].map(
            (stateName) => (
        ...
        >
          ...
          {baseOptions.map((loc) => (
            <option key={loc.id} value={loc.military_base}>
              {loc.military_base}
            </option>
          ))}
        </select>
        ...
        <div className="priority-select-wrapper">
          ...
          <div className="priority-help">
            ...
            <div className="help-text">
              ...
              <ul>
                <li>
                  <strong>1 : </strong> needs fixed within 24 hours (e.g. burst
                  pipe, HVAC outage)
                </li>
                <li>
                  <strong>2 : </strong> Should be fixed within the week (e.g.
                  broken outlet, door issue)
                </li>
                <li>
                  <strong>3 : </strong> Should be fixed within 30 days (e.g.
                  paint touch-up, minor repairs)
                </li>
              </ul>
            </div>
          </div>
